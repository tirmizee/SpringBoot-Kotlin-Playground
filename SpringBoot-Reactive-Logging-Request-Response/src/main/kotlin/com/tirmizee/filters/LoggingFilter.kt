...
class LoggingFilter : WebFilter {
    ...
    override fun filter(exchange: ServerWebExchange, chain: WebFilterChain): Mono<Void> {
        ...
        return chain.filter(RequestResponseExchange(exchange, requestId))
    }

}

class RequestResponseExchange(exchange: ServerWebExchange, private val requestId: String): ServerWebExchangeDecorator(exchange) {


    override fun getRequest(): ServerHttpRequest = RequestLoggingDecorator(super.getRequest(), requestId)
    override fun getResponse(): ServerHttpResponse = ResponseLoggingDecorator(super.getResponse(), requestId)
...
class ResponseLoggingDecorator(delegate: ServerHttpResponse, private val requestId: String) : ServerHttpResponseDecorator(delegate) {
    ...
    override fun writeWith(body: Publisher<out DataBuffer>): Mono<Void> {
        ...
        buffer.doOnNext { dataBuffer ->
            ...
            LoggingFilter.log.info("Response $requestId : status=${delegate.statusCode}, time=${System.currentTimeMillis()}, payload=$bodyBuffer")
        }
